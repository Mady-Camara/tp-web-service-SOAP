...
public class EtudiantPersistance {
    @WebMethod(operationName = "getEtudiants")
    public List<Etudiant> getEtudiants() {
        List<Etudiant> liste = new ArrayList<>();
        String query = "select id,prenom,nom from etudiant;";
        try {
            Etudiant e = new Etudiant();
            Class.forName("com.mysql.jdbc.Driver");
            Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/mglsi_etudiants", "root", "");
    ...
    public Etudiant getEtudiant(@WebParam(name = "id") int id) {
        ...
        String query = "select id,prenom,nom from etudiant where id =?";
    ...
    public Boolean ajouterEtudiant(@WebParam(name = "prenom") String prenom, @WebParam(name = "nom") String nom) {
        String query = "Insert into etudiant (prenom, nom) values (?,?)";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/mglsi_etudiants", "root", "");
            PreparedStatement preparedStmt = connect.prepareStatement(query);
    ...
    public Boolean modifierEtudiant(@WebParam(name = "id") int id, @WebParam(name = "prenom") String prenom,
        ...
        String query = "update etudiant set prenom = ?, nom = ? where id =?;";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/mglsi_etudiants", "root", "");
            PreparedStatement preparedStmt = connect.prepareStatement(query);
